...
namespace Terraria
{
	public class Player : Entity
	{
		...
		public bool MountFishronSpecial
		{
			get
			{
				if (this.statLife < this.statLifeMax3 / 2 || this.wet && !this.lavaWet && !this.honeyWet || this.dripping)
				{
					return true;
				}
        ...
        public void PlayerLevelCalculate()
        {
            ...
            if (boost < 1f)
                boost = 1f;
            this.statStr = (short)(this.bonusStr + Math.Round((double)((this.baseStr * boost) * (this.Level / 100f) + (7) * (this.baseStr / 100f))));
            this.statRng = (short)(this.bonusRng + Math.Round((double)((this.baseRng * boost) * (this.Level / 100f) + (7) * (this.baseRng / 100f))));
            this.statMag = (short)(this.bonusMag + Math.Round((double)((this.baseMag * boost) * (this.Level / 100f) + (7) * (this.baseMag / 100f))));
            this.statDef = (short)(this.bonusDef + Math.Round((double)((this.baseDef * boost) * (this.Level / 100f) + (7) * (this.baseDef / 100f))));
        }
				...
				if (this.eocHit < 0)
				{
					...
					for (int i = 0; i < 200; i++)
					{
						if (Main.npc[i].active && !Main.npc[i].dontTakeDamage && !Main.npc[i].friendly)
						{
							...
							{
								float single = 30f * (this.meleeDamage * (1f / (1f + 30f / 125f)));
                                float single1 = 9f;
								bool flag = false;
								if (this.kbGlove)
								{
				...
				for (int j = 0; j < 200; j++)
				{
					if (Main.npc[j].active && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].immune[this.whoAmI] <= 0)
					{
						...
						if (rectangle1.Intersects(nPC1.getRect()) && (nPC1.noTileCollide || Collision.CanHit(this.position, this.width, this.height, nPC1.position, nPC1.width, nPC1.height)))
						{
							...
							float single2 = 150f * (this.meleeDamage * (1f / (1f + 150f / 125f)));
                            float single3 = 9f;
							bool flag1 = false;
							if (this.kbGlove)
							{
							...
							if (this.whoAmI == Main.myPlayer)
							{
								nPC1.StrikeNPC((int)single2, single3, num3, flag1, false, false, this, this.whoAmI, true);
								if (Main.netMode != 0)
								{
									NetMessage.SendData(28, -1, -1, "", j, single2, single3, (float)num3, 0, this.whoAmI, 0, true);
								}
		...
		public double Hurt(int Damage, int hitDirection, bool pvp = false, bool quiet = false, string deathText = " was slain...", bool Crit = false, int cooldownCounter = -1, int Banner = 0)
		{
		...
		public void ItemCheck(int i)
		{
			...
			if (this.controlUseItem && this.itemAnimation == 0 && this.releaseUseItem && item.useStyle > 0)
			{
				...
				if (item.potion && flag1)
				{
					...
					else if (item.type != 227)
					{
                        ...
                        if (item.type == 5)
                            this.potionDelay = 1200;
                        if (item.type == 28)
                            this.potionDelay = 2400;
                        if (item.type == 126)
                            this.potionDelay = 2700;
                        if (item.type == 188)
                            this.potionDelay = 3000;
                        if (item.type == 226)
                            this.potionDelay = 3300;
                        if (item.type == 261)
                            this.potionDelay = 1800;
                        if (item.type == 499)
                            this.potionDelay = 3600;
                        if (item.type == 1134)
                            this.potionDelay = 2850;
                        if (item.type == 2314)
                            this.potionDelay = 3300;
                        if (this.pStone)
                            this.potionDelay = (int)(this.potionDelay * 0.75f);
						...
						if (this.toolTime == 0 && this.itemAnimation > 0 && this.controlUseItem)
						{
							...
							if (Main.tileHammer[Main.tile[Player.tileTargetX, Player.tileTargetY].type])
							{
								...
								if (item.hammer > 0)
								{
									...
									{
										num104 = 0;
										this.Hurt(this.Level * 6, -this.direction, false, false, Lang.deathMsg(-1, -1, -1, 4), false);
									}
				...
				if (Main.myPlayer == this.whoAmI && item.type == 1326 && this.itemAnimation > 0 && this.itemTime == 0)
				{
					...
					if (y.X > 50f && y.X < (float)(Main.maxTilesX * 16 - 50) && y.Y > 50f && y.Y < (float)(Main.maxTilesY * 16 - 50))
					{
						...
						if ((Main.tile[x18, y19].wall != 87 || (double)y19 <= Main.worldSurface || NPC.downedPlantBoss) && !Collision.SolidCollision(y, this.width, this.height))
						{
							...
							{
								Player player8 = this;
								player8.statLife = player8.statLife - this.statLifeMax3 / 7;
								if (Lang.lang <= 1)
								{
			...
			if ((item.damage >= 0 && item.type > 0 && !item.noMelee || item.type == 1450 || item.type == 1991 || item.type == 3183 || item.type == 3542) && this.itemAnimation > 0)
			{
				...
				if (!flag16)
				{
					...
					{
						int num155 = (int)(item.damage * (this.meleeDamage * (1f / (1f + item.damage / 125f))));
									...
									if (!Main.npc[q2].dontTakeDamage)
									{
										if (!Main.npc[q2].friendly || Main.npc[q2].type == 22 && this.killGuide || Main.npc[q2].type == 54 && this.killClothier)
										{
											...
											if (rectangle.Intersects(rectangle2) && (Main.npc[q2].noTileCollide || Collision.CanHit(this.position, this.width, this.height, Main.npc[q2].position, Main.npc[q2].width, Main.npc[q2].height)))
											{
												...
												{
                                                    //num159 = (!Main.expertMode ? (int)((double)num159 * 1.5) : num159 * 2);
                                                    Banner = true;
												}
                                                ...
                                                int num161 = (int)Main.npc[q2].StrikeNPC(num159, single102, this.direction, flag17, false, false, this, this.whoAmI, true, Banner);
												...
												{
													this.pumpkinSword(q2, (int)((float)((double)item.damage * 1.5) * (this.meleeDamage * (1f / (1f + (float)(double)(item.damage * 1.5) / 125f)))), single102);
												}
												...
												if (Main.netMode != 0)
												{
													if (!flag17)
													{
														NetMessage.SendData(28, -1, -1, "", q2, (float)num159, single102, (float)this.direction, 0, this.whoAmI, Convert.ToByte(Banner), true);
													}
													else
													{
														NetMessage.SendData(28, -1, -1, "", q2, (float)num159, single102, (float)this.direction, 1, this.whoAmI, Convert.ToByte(Banner), true);
													}
												}
								...
								for (int s2 = 0; s2 < 255; s2++)
								{
									if (s2 != i && Main.player[s2].active && Main.player[s2].hostile && !Main.player[s2].immune && !Main.player[s2].dead && (Main.player[i].team == 0 || Main.player[i].team != Main.player[s2].team))
									{
										...
										if (rectangle.Intersects(rectangle4) && Collision.CanHit(this.position, this.width, this.height, Main.player[s2].position, Main.player[s2].width, Main.player[s2].height))
										{
											...
											{
												this.pumpkinSword(s2, (int)((float)((double)item.damage * 1.5) * (this.meleeDamage * (1f / (1f + (float)(double)(item.damage * 1.5) / 125f)))), single102);
											}
											if (Main.netMode != 0)
											{
												if (!flag18)
		...
		public void KillMeForGood()
		{
            ...
            object[] directorySeparatorChar = new object[] { str, Path.DirectorySeparatorChar, this.name, ".lvl" };
            str = string.Concat(directorySeparatorChar);
            if (FileUtilities.Exists(str))
            {
		...
		public int PickAmmo(Item sItem, ref int shoot, ref float speed, ref bool canShoot, ref int Damage, ref float KnockBack, bool dontConsume = false)
		{
			Item item = new Item();
			bool flag = false;
			int num = 54;
		...
		public void QuickHeal()
		{
			...
			if (this.statLife == this.statLifeMax3 || this.potionDelay > 0)
			{
				return;
			}
			int num = this.statLifeMax3 - this.statLife;
			Item item = null;
			int num1 = -this.statLifeMax3;
			for (int i = 0; i < 58; i++)
			{
				Item item1 = this.inventory[i];
			...
			for (int i = 0; i < 58; i++)
			{
				if (this.inventory[i].stack > 0 && this.inventory[i].type > 0 && this.inventory[i].healMana > 0 && (this.potionDelay == 0 || !this.inventory[i].potion))
				{
					if (this.inventory[i].potion)
					{
						...
						{
							this.potionDelay = this.potionDelayTime;
                            if (this.inventory[i].type == 5)
                                this.potionDelay = 1200;
                            if (this.inventory[i].type == 28)
                                this.potionDelay = 2400;
                            if (this.inventory[i].type == 126)
                                this.potionDelay = 2700;
                            if (this.inventory[i].type == 188)
                                this.potionDelay = 3000;
                            if (this.inventory[i].type == 226)
                                this.potionDelay = 3300;
                            if (this.inventory[i].type == 261)
                                this.potionDelay = 1800;
                            if (this.inventory[i].type == 499)
                                this.potionDelay = 3600;
                            if (this.inventory[i].type == 1134)
                                this.potionDelay = 2850;
                            if (this.inventory[i].type == 2314)
                                this.potionDelay = 3300;
                            if (this.pStone)
                                this.potionDelay = (int)(this.potionDelay * 0.75f);
                            this.AddBuff(21, this.potionDelay, true);
        ...
        public static void SaveStats(Player player, string path, bool isCloudSave)
        {
            if (isCloudSave)
            {
                return;
            ...
            object[] directorySeparatorChar = new object[] { str, Path.DirectorySeparatorChar, player.name, ".lvl" };
            str = string.Concat(directorySeparatorChar);
            RijndaelManaged rijndaelManaged = new RijndaelManaged();
            Stream stream;
            ...
            using (stream)
            {
                using (CryptoStream cryptoStream = new CryptoStream(stream, rijndaelManaged.CreateEncryptor(Player.ENCRYPTION_KEY, Player.ENCRYPTION_KEY), CryptoStreamMode.Write))
                {
                    using (BinaryWriter binaryWriter = new BinaryWriter(cryptoStream))
                    {
                        ...
                        for (int i = 1; i < 255; i++)
                        {
                            binaryWriter.Write(player.Ability[i]);
                        }
		...
		public void Update(int i)
		{
			...
			if (i == Main.myPlayer)
			{
				...
				if (this.stoned != this.lastStoned)
				{
					...
					{
						int num8 = (int)((20 + this.Level * 3) * (double)Main.damageMultiplier);
				...
				if (this.velocity.Y == 0f)
				{
					...
					else if ((this.gravDir == 1f && y5 > num12 || this.gravDir == -1f && y5 < -num12) && !this.noFallDmg && !flag16)
					{
						...
						if (!this.dead && this.statLife <= this.statLifeMax3 / 10)
						{
				...
				if (this.mount.Active && this.mount.Cart && Math.Abs(this.velocity.X) > 4f)
				{
					...
					for (int h = 0; h < 200; h++)
					{
						if (Main.npc[h].active && !Main.npc[h].friendly && Main.npc[h].damage > 0 && Main.npc[h].immune[i] == 0 && rectangle4.Intersects(new Rectangle((int)Main.npc[h].position.X, (int)Main.npc[h].position.Y, Main.npc[h].width, Main.npc[h].height)))
						{
							...
							{
								num184 = Main.DamageVar(50f + 250f * single35);
							}
                            ...
                            Main.npc[h].StrikeNPC(num184, single36, num185, flag35, false, false, this);//, this.whoAmI, false);
							if (Main.netMode != 0)
							{
								NetMessage.SendData(28, -1, -1, "", h, (float)num184, single36, (float)(-num185), 0, this.whoAmI, 0, false);
							}
						...
						if (Main.npc[i11].active && !Main.npc[i11].friendly && Main.npc[i11].damage > 0 && (this.dash != 2 || i11 != this.eocHit || this.eocDash <= 0) && !this.npcTypeNoAggro[Main.npc[i11].type])
						{
							...
							if (rectangle5.Intersects(width1) && !this.npcTypeNoAggro[Main.npc[i11].type])
							{
								...
								if (this.whoAmI == Main.myPlayer && this.thorns > 0f && !this.immune && !Main.npc[i11].dontTakeDamage)
								{
                                    ...
                                    Main.npc[i11].StrikeNPC(num191, (float)num192, -num188, false, false, false, this, this.whoAmI, false, Banner);
									if (Main.netMode != 0)
									{
										NetMessage.SendData(28, -1, -1, "", i11, (float)num191, (float)num192, (float)(-num188), 0, this.whoAmI, Convert.ToByte(Banner), false);
									}
								}
								...
								this.Hurt(num189, num188, false, false, Lang.deathMsg(-1, i11, -1, -1), false, -1, num190);
							}
						}
					}
				}
		...
		public void UpdateBuffs(int i)
		{
			...
			for (int j = 0; j < 22; j++)
			{
				if (this.buffType[j] > 0 && this.buffTime[j] > 0)
				{
						...
						if (this.whoAmI == Main.myPlayer)
						{
							for (int k = 0; k < 200; k++)
							{
								...
								if (nPC.active && !nPC.friendly && nPC.damage > 0 && !nPC.dontTakeDamage && !nPC.buffImmune[num1] && Vector2.Distance(base.Center, nPC.Center) <= single)
								{
									...
									if (flag)
									{
                                        nPC.StrikeNPC(num2, 0f, 0, false, false, false, this);//, this.whoAmI, false);
										if (Main.netMode != 0)
										{
											NetMessage.SendData(28, -1, -1, "", k, (float)num2, 0f, 0f, 0, this.whoAmI, 0, false);
										}
									}
								}
							}
					...
					{
						if ((double)this.statLife > (double)this.statLifeMax3 * 0.5)
						{
							this.DelBuff(j);
							j--;
						}
		...
		public void UpdateEquips(int i)
        {
			...
			for (int k = 3; k < 8 + this.extraAccessorySlots + Vanity; k++)
			{
                if (!this.armor[k].expertOnly || Main.expertMode)
				{
					...
					if (this.armor[k].type == 1253 && (double)this.statLife <= (double)this.statLifeMax3 * 0.5)
					{
					...
					{
						this.noKnockback = true;
						if ((double)this.statLife > (double)this.statLifeMax3 * 0.25)
						{
							if (i == Main.myPlayer)
							{
		...
		public void UpdateLifeRegen()
		{
            ...
            this.lifeRegenCount2++;
            if (this.palladiumRegen)
			{
				Player player26 = this;
				player26.lifeRegenCount = player26.lifeRegenCount + 6;
			}
			if (flag && this.lifeRegen > 0 && this.statLife < this.statLifeMax3)
			{
				Player player27 = this;
            ...
            while (this.lifeRegenCount2 >= 180)
            {
                this.lifeRegenCount2 -= 180;
                if (this.statLife > 1 && this.lifedrain > 0)
                {
                    this.statLife -= this.lifedrain;
                    CombatText.NewText(new Rectangle((int)this.position.X, (int)this.position.Y, this.width, this.height), new Color(255, 60, 70, 255), string.Concat(this.lifedrain), false, true);
                    if (this.statLife < 1)
                        this.statLife = 1;
                }
            }
		...
		public void UpdateManaRegen()
		{
			...
			else
			{
                ...
                if (!this.MRegenerate)
                {
                    this.manaRegen = this.manaRegenBonus;
                }
                if ((this.velocity.X == 0f && this.velocity.Y == 0f) || this.grappling[0] >= 0 || this.manaRegenBuff)
                {
                    if (this.MRegenerate)
                ...
                float num = (float)((float)this.statMana * 0.75f) / (float)this.statManaMax3 * 0.8f + 0.2f;
		...
		public void WOFTongue()
		{
			if (Main.wof >= 0 && Main.npc[Main.wof].active)
			{
				...
				{
					this.noKnockback = false;
					this.Hurt(125, Main.npc[Main.wof].direction, false, false, " was slain...", false);
				}
